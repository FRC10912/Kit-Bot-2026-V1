...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    operatorController.leftBumper()
        .whileTrue(ballSubsystem.runEnd(() -> ballSubsystem.intake(), () -> ballSubsystem.idle_feeder()));
    // While the right bumper on the operator controller is held, spin up for 1
    // second, then launch fuel. When the button is released, stop.
    operatorController.rightBumper()
        .whileTrue(ballSubsystem.spinUpCommand().withTimeout(SPIN_UP_SECONDS)
            .andThen(ballSubsystem.launchCommand())
            .finallyDo(() -> ballSubsystem.idle_feeder()));
    ...
    operatorController.b().onTrue(ballSubsystem.run(() -> override_shooting_to_max = true));
    operatorController.b().onFalse(ballSubsystem.run(() -> override_shooting_to_max = false));
    ...
    driveSubsystem.setDefaultCommand(
        driveSubsystem.driveArcade(
            ...
            () -> -driverController.getRightX() * ROTATION_SCALING * drive_speed_multiplier + (operatorController.getLeftTriggerAxis() * x_angle * auto_aim_kp)));
  }
